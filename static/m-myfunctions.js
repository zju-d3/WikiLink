...
function SHOW_UPDATE_FORCE(dataset, born){
    ...
    console.log("CLIENT_NODES", CLIENT_NODES);
    console.log("CLIENT_EDGES", CLIENT_EDGES);
    ...
    const gData = {
        ...
        links: CLIENT_EDGES 
        .map(item=>({
            source:item.source.wid,
            target:item.target.wid
        }))
    };
    ...
    const coreNode = gData.nodes[0];

    // add nodes only when PATHS > 1 node
    if(PATHS.length > 1){

        nodeSet = PATHS[0].ids
        linkSet = PATHS[0].ids.slice(1)
        linkSet = linkSet.map((item, index)=>{
            return {source:PATHS[0].ids[index], target:item}
        });

        gData.nodes.map(node =>{
            if(nodeSet.includes(node.id)){
                highlightNodes.add(node);
            }
        });

        gData.links.map(link =>{
            link_id = {source:link.target, target:link.source}
            if(linkSet.some(link=> link.source == link_id.source && link.target == link_id.target)){
...
function highlight_nodespaths(dataset, data){
...
function highlight_nodespaths(dataset, data){
    ...
    const gData = {
        ...
        links: CLIENT_EDGES
        .map(item=>{
            if (item.source.wid != item.target.wid)
                return {
                    source:item.source.wid,
                    target:item.target.wid,
                    curvature: 0.8,
    ...
    // add highlight nodes only when PATHS > 1 node
    if(PATHS.length > 1){

        nodeSet = dataset.paths[0];

        linkSet = dataset.paths[0].slice(1);
        linkSet = linkSet.map((item, index)=>{
            return {source:nodeSet[index], target:item}
        });
        ...
        gData.links.map(link =>{
            link_id = {source:link.target, target:link.source}
            if(linkSet.some(link=> link.source == link_id.source && link.target == link_id.target ||
                link.target == link_id.source && link.source == link_id.target)){
                highlightLinks.add(link)
            }
        });
    }
    ...
    (document.getElementById('3d-graph'))
        ...
        .nodeColor(node => highlightNodes.has(node) ? node === coreNode ? 'rgb(255,0,0,1)' : 'rgb(255,0,0,1)' : 'rgba(255,160,0,0.8)')
        .nodeThreeObject(node => {
            const nodeEl = document.createElement('div');
        ...
        .linkColor(link => highlightLinks.has(link) ? 'rgb(255,0,0,1)':'#aaa')
        .linkOpacity(0.5)
    ...
    // if more than 1 node, fit to canvas when engine stops
    if(dataset.paths.length != 0){
...
function generator_update_graphAndPanel(info,bornplace,znodes,queries){
    ...
    console.log("generator_update_graphAndPanel");
    ...
    d3.json('/generator/'+JSON.stringify(info),function(error,data){
        ...
        highlight_nodespaths(highlights, data);
        //zoom to the node
        ZoomToNodes(z_nodes); 
        //update the information panel here
        update_informationPanel(data.paths,data.position, data);
    });
};
...
function update_informationPanel(paths,position,data){

    console.log("update_informationPanel");
...
function Colorized_Clusters(clusters){
    ...
    console.log("Colorized_Clusters");
